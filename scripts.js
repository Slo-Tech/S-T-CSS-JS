...
(function($) {
	...
	function endDrag(e) {
		...
		textarea.focus();
		$.cookie('form_height', $(textarea).height(), {expires: 7});
		textarea = null;
		staticOffset = null;
		iLastMousePos = 0;
...
(function ($)
{
	...
	{		
		/**
		 * @public
		 * @return the target jQuery object for further chaining.
		 * @type jQuery
		 */
		entityMarker: function (elements, destinationUri, sendPartial)
		{
			/* Reference to this, used in closures */
			var self = this;

			/* Explicit boolean type cast */
			sendPartial = sendPartial ? true : false;
	
			/**
			 * The template method to be initialized upon the first call of
			 * #initSelector(). After initialization this method represents
			 * a common interface to access the current selection in the
			 * document (if any).
			 * 
			 * @private
			 */
			...
			/**
			 * Initializes the #getSelectedText() function with the first
			 * available method.
			 * 
			 * @private
			 */
			var initSelector = function ()
			{
			    ...
			    {
					getSelectedText = function () {return document.getSelection();};
			    }
			    else if (document.selection)
			    {
			    	getSelectedText = function () {return document.selection.createRange().text;};
			    }
			    else return false;
			};
			...
			/**
			 * Reference to the toolbar container for faster lookup.
			 * 
			 * @private
			 */
			var toolbar;
			...
			/**
			 * Callback function for entity marker actions from the toolbar.
			 * 
			 * @param {MouseEvent} e
			 * @param {Object} properties
			 * @private
			 */
			var markHandler = function (e, properties)
			{
				e.stopPropagation();
				e.preventDefault();
				...
				var newNode = document.createElement(properties.tag);
                        $(newNode).attr({title: properties.name});

				range.surroundContents(newNode);
				
				var jNewNode = $(newNode);
				
				toolbar.fadeOut();
				
				var match;
				var parents = jNewNode.parents();
				var parentLength = parents.length;
				
				jNewNode.unbind('click');
				jNewNode.click(function (e)
				{
					e.stopPropagation();
					e.preventDefault();
					...
					element.before(element.html()).empty();
				});
				...
				if (match && sendPartial)
				{
					$.post(destinationUri, 'taggedDoc=' + escape(match.html()));
				}
			};
			
			/**
			 * Callback function for entity removal actions from the toolbar.
			 * 
			 * @param {MouseEvent} e
			 */
			var removeHandler = function (e)
			{
				...
				var rawfragment = range.extractContents(); 
				var fragment = $(rawfragment.childNodes);
				...
				fragment.each(function (i)
				{
					...
					for (key in elements)
					{
						if (elements.hasOwnProperty(key) && element[0] && element[0].tagName
						&& elements[key].tag.toLowerCase() == element[0].tagName.toLowerCase())
						{							
							for (var i = 0; i < element[0].childNodes.length; i++)
							{
								finalFragment.appendChild(element[0].childNodes[i]);
							}
					...
					if (!changed)
					{
						finalFragment.appendChild(this);
					}
				});
			...
			/**
			 * Initializes the marker toolbar.
			 * Creates a clickable button for every entry in the elements
			 * parameter and an additional button for removing existing entities.
			 * 
			 * After initialization this method may be used as a getter for the
			 * toolbar element. Such usage is encourage instead of manually
			 * querying it.
			 * 
			 * Note: this operations creates a new <styles> tag in the body
			 * element of this document followed by this toolbar's containing
			 * <div>.
			 * 
			 * @private
			 ...
			 * @type jQuery
			 */
			var initToolbar = function (elements)
			{
				if (toolbar == null)
				{
					toolbar = $('<div>').css(
					...
					for (key in elements)
					{
						...
						var el = elements[key];
						var listItem = $('<li>').attr({title: el.name, rel: el.tag}).text(el.button);
						...
						if (elements[key].style != null)
						{
							listItem.css(elements[key].style);
						}
	
						listItem.mousedown(function (props){return function (e){markHandler(e, props);};}(el));
						list.append(listItem);
					}
					
					var removeToolbar = $('<div/>');

					var removeList = $('<ul>').css({
						'list-style-image': 'none',
						'list-style-position': 'outside',
						'list-style-type': 'none',
						margin: '4px 0'
					...
					removeToolbar.append(removeList);
	
					markerToolbar.append(list);
					
					toolbar.append(markerToolbar);
					toolbar.append(removeToolbar);
			...
			/**
			 * Fades in the toolbar (if initialized) in close proximity to the
			 * location specified by the <code>x</code> and <code>y</code>
			 * arguments.
			 * 
			 * @param {Integer} x
			 * @param {Integer} y
			 ...
			 * @private
			 */
			var showToolbar = function (x, y, remove)
			{
				var index = remove ? 1 : 0;
				var bar = initToolbar(elements);
				
				var toolbarHolder = $('div', bar).eq(index);
				var other = $('div', bar).eq(1 - index);
				
				toolbarHolder.show();
				other.hide();
				bar.css({top: y - 20 + 'px', left: x + 40 + 'px'});
				bar.fadeIn(120);
			};
			
			/**
			 * Callback used to detect selection events in the registered jQuery
			 * objects.
			 * 
			 * @private
			 * @param {MouseEvent} e
			...
			{
				var selectionBody = getSelectedText();
	
				if (selectionBody.toString().length > 0)
				{
			...
			for (key in elements)
			{
				if (elements.hasOwnProperty(key) && elements[key].style != null)
				{
					var styleKey;
					
					var tagStyles = [];
					
					for (styleKey in elements[key].style)
					{
						if (elements[key].style.hasOwnProperty(styleKey))
						{
							tagStyles.push([styleKey, ':', elements[key].style[styleKey]].join(' '));
						}
					}
					
					tagStyles.push('cursor: crosshair;');
					
					styleHtml.push([elements[key].tag, '{', tagStyles.join(';'), '}'].join(' '));
				}
			}
	
			$('head').append($('<style>').attr('type', 'text/css').html(styleHtml.join('\n')));
			
			/* Allow for further chaining */
			return this;
		}
	});
}) (jQuery);
...
$(function ()
{
	var entities = [{name: 'Person', tag: 'person', button: 'P', style: {color: 'pink'}},
	                {name: 'Time', tag: 'datetime', button: 'T', style: {color: 'lime'}},
	                {name: 'Geo', tag: 'geo', button: 'G', style: {color: 'brown'}},
			{name: 'Money', tag: 'money', button: 'M', style: {color: 'red'}},
	                {name: 'Company/Brand', tag: 'company', button: 'C', style: {color: 'cyan'}}];

	$('article div.besediloNovice').entityMarker(entities,	'http://slo-tech.com', true);
});
/* / Example */